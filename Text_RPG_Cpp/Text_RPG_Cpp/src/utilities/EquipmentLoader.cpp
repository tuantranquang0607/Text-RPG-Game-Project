...
// Function to create stat modifier from an XML element
...
// Constructor for EquipmentLoader
EquipmentLoader::EquipmentLoader(const std::string& filepath, bool weapons) : m_sFilepath(filepath), m_bWeaponLoader(weapons)
{
    m_pXMLDoc = std::make_unique<tinyxml2::XMLDocument>(); // Create a new XML document
}

// Destructor for EquipmentLoader
...
std::shared_ptr<Equipment> EquipmentLoader::CreateObjectFromFile(const std::string& objName)
{
	...
	while (pItem) 
	{
		...
		newEquipment = ItemCreator::CreateEquipment(equipType, weapon_props, armor_props, stat_mod, equipName, desc, buy_price);

		return newEquipment;
	}

	// If we get here, we did not create an item
	TRPG_ERROR("Item - [" + objName + "] - Does not exist"); // Log error if the item does not exist
	assert(newEquipment); // Assert that newEquipment is not null
